 ...
 */
public class IncluirBoletoPessoaFisicaService {
     ...
     * @return
     */
    public Servico_saida_negocial_Type incluirBoletoPessoaFisica(DadosDto dto) {
        try {
    ...
    private Dados_entrada_Type getDadosToIncludeBoleto(DadosDto dto) {
        ...
        String cpf = dto.getIncluiBoletoDtoObject().getTituloDtoObject().getPagadorDtoObject().getCpf();
        dto.getIncluiBoletoDtoObject().getTituloDtoObject().getPagadorDtoObject().setCpf(utilBody.substitueCaracterEspeciaisCnpjCpf(cpf));
        ...
        if (endereco != null) {
            if (endereco.getLOGRADOURO().length() > 35 && endereco.getLOGRADOURO() != null) {
                dadosEntradaType.getINCLUI_BOLETO().getTITULO()
                        .getPAGADOR().getENDERECO()
                        .setLOGRADOURO(endereco.getLOGRADOURO().substring(0, 35));
            }
            if (endereco.getBAIRRO().length() > 10 && endereco.getBAIRRO() != null) {
                dadosEntradaType.getINCLUI_BOLETO().getTITULO()
                        .getPAGADOR().getENDERECO()
                        .setBAIRRO(endereco.getBAIRRO().substring(0, 10));
            }
            if (endereco.getCIDADE().length() > 10 && endereco.getCIDADE() != null) {
                dadosEntradaType.getINCLUI_BOLETO().getTITULO()
                        .getPAGADOR().getENDERECO()
                        .setCIDADE(endereco.getCIDADE().substring(0, 10));
            }
            if (endereco.getUF().trim().length() > 2 && endereco.getUF() != null) {
                dadosEntradaType.getINCLUI_BOLETO().getTITULO()
                        .getPAGADOR().getENDERECO()
        ...
        String nome = dadosEntradaType.getINCLUI_BOLETO().getTITULO().getPAGADOR().getNOME();
        if (nome.length() > 40 && nome != null) {
            dadosEntradaType.getINCLUI_BOLETO().getTITULO().getPAGADOR().setNOME(nome.substring(0, 40));
        }
